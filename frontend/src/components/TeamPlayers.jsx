...
export const TeamPlayers = () => {
  ...
  const getplayersByTeamId = async () => {
    const res = await axios.get("/team/getplayer/" + teamid);
    console.log(res.data.team);
    setplayers(res.data.team);
  };
    ...
    <>
      ...
      <div style={{ marginTop: 150 }} className="container-xxl py-5">
        <div className="container">
          ...
          >
            <h5 className="section-title ff-secondary text-center text-primary fw-normal" style={{fontSize:25}}>
              team
            </h5>
            <h1 className="mb-5">Team Players </h1>
            <div>
              <button
                onClick={() => {
                  showDropDownHandler();
                    ...
                    return(
                      ...
                      <div className="w-100 d-flex flex-column text-start ps-4">
                        <h5 className="d-flex justify-content-between border-bottom pb-2">
                          <span>{player.user.firstName}</span>
                          <span>{player.user.lastName}</span>
                          <span className="text-primary">{player.playerRole.name}</span>
                        </h5>
                        <small className="fst-italic">
                          {player.playerRole.description}
                        </small>
                      </div>
                    </div>
                    )
